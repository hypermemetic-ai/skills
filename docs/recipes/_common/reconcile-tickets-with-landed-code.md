...
5. Chase every supersession to its terminal state: canceled ticket → pointer comment → parent DAG updated → **orphan draft PRs closed**.
...
- A cancel comment that promises "the DAG drops this node" is not the DAG dropping the node — *earned by:* a canceled concurrency-guard build whose execution ticket still routed `B7 → B8 → B4` two days later, and whose draft PR (#985) stayed open after the comment said it could close.
...
- A blocking relation is not a contract — *earned by:* a milestone blocked on an upstream "output" (`AuthedIdentity`) that no upstream ticket named; the Linear edge existed, the producer-side `Provides` didn't (a missing root, found only by reading the producer's scope).
