...
A markdown file in `plans/<EPIC>/<EPIC>-<N>.md` with YAML frontmatter. Status is always `Pending` — only the user promotes to `Ready`. Claude must never set a ticket to `Ready` on creation. Implementation must not begin on `Pending` tickets.
...
| `Ready` | Approved, no blockers, ready for implementation | Human (after review) |
