...
12. **Build + tests must pass to reach Complete.** Every ticket's acceptance criteria must include "`cargo build` and `cargo test` (or the workspace-language equivalent) pass green end-to-end for the workspace the ticket affects." A ticket can be Ready (approved, implementation can start) while pre-existing issues block the integration gate, but it stays Ready — not Complete — until the gate passes. If pre-existing issues prevent the gate from ever passing as-is, write a dependency ticket that fixes the blocker and add it to `blocked_by`. Flipping to Complete before the integration gate is green is a contract violation: downstream consumers assume "Complete" means the workspace is in a shippable state, and that assumption cascades into later epics' promotion decisions.

    **How this interacts with pre-existing blockers:** if ticket A's own work is done and committed but a separate pre-existing issue prevents the workspace from building, A stays Ready. File a ticket B that fixes the blocker, add B to A's `blocked_by`. When B lands, retry A's acceptance criteria — if the integration gate is now green, flip A to Complete. A and B can ship in a single commit run if the implementor wants to batch.

    **Retrofit policy:** if a past ticket was flipped to Complete without the integration gate passing, flip it back to Ready, add the missing `blocked_by`, and carry it through the discipline cleanly. History gets a small correction; future tickets avoid the same error.
