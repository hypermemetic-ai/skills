...
graph TD
  ...
  SCOPE --> EXEC["3 · execution ticket<br/>owns the work as children; refs the scope"]
  EXEC --> Q{"4 · one self-contained diff,<br/>or many?"}
  Q -->|"one diff"| BUILD["a single build ticket<br/>Pending → Ready → active → … → done"]
...
2. **Finalize the scope ticket** ([scope-ticket](../ticketing/scope-ticket.md)): the canonical *what/why*, the `## Language` it leans on, and the **interface contract**. It's the milestone's single entry point (done-by-definition once it has children, never archived).
3. **Create the execution ticket** — the [execution ticket](../ticketing/execution-ticket.md) that will own the work as children; it references the scope. Don't pre-draw its DAG.
...
- In the tracker: a **scope ticket** + an **execution ticket** for the milestone, the execution owning dependency-ordered spike and build tickets as children — all created **Pending**; the human ratifies them onward.
...
What does the system look like when this execution ticket is done? Not the steps — the destination. Write it as the execution ticket's `## Goal`.

### 2. Work backwards from the end state

What's the last thing that must happen? That's the leaf. What does it depend on? Those are its dependency edges (depends-on). Keep walking until you reach tickets with no dependencies — the roots.
...
- `low` — significant unknowns; **must be blocked by a spike**. Ratifying a `low` ticket to **Ready** without one is a planning error.
...
The plan is two ticket bodies, iterated in place. The **scope ticket** carries the what/why: `## Language` · decided shape · edges · the interface contract ([scope-ticket](../ticketing/scope-ticket.md)). The **execution ticket** carries the how: `## Execution DAG` · `## The work` (each child linked, one-line scope) · `## Decision gates` · `## Exit` · `## Completion gate` · `## Status` ([execution-ticket](../ticketing/execution-ticket.md)). Dependencies are dependency edges, derivable from the tickets' `## Provides`/`## Consumes`.
...
- **Pending until ratified.** All tickets are created **Pending**; only the human moves them onward (**Ready** = eligible to build).
- **The tracker is the source of truth.** The scope and execution ticket bodies are the canonical documents — decide, argue, and iterate there, and update them in the same unit of work as any structural change.
...
A child of the execution ticket, titled `S<N> · Spike: <question>`, with a dependency edge to each build its evidence defines. Body:
...
Some decisions outlive the execution ticket that made them. Those live in the worked-on repo at **`docs/adr/NNNN-slug.md`** — sequential numbering (scan for the highest, increment), so they're **citable** from tickets, reviews, and `## Evidence` ("per ADR-0007").
...
ADRs are minted two ways: **live**, when a decision crystallizes during grilling/planning and passes the test; and — the main road — by the **distill pass within execution**: a *distill node* over the execution graph (the whole graph, or any closed sub-section), run mid-execution as tracks close and **always before the execution is concluded complete** ([execution-ticket](../ticketing/execution-ticket.md) → "Distill nodes"). **Scoping doesn't mint ADRs — it consumes them**: the next scope reads the records and cites "per ADR-NNNN" instead of relitigating. They're one of the three **durable records** ([methodology](../methodology/SKILL.md) → "Durable records"): the glossary holds the *language*, ADRs hold the *decisions*, recipes hold the *procedures*. Execution-ticket decision gates still record `DECIDED: …` + evidence as always — an ADR is for the subset a future stranger must find *without* reading this execution ticket.

## The milestone in the tracker

**The tracker is the source of truth — the plan lives in the tickets and is iterated there.** Each milestone gets two ticket kinds:

- **Scope ticket** — the canonical, always-current view: the *what/why*, the language, the interface contract. The milestone's single entry point, diagram-led. **Done by definition — but only once it has produced ≥1 work ticket**; **never archived** (this is a living reference, not abandoned work). Format → [scope-ticket](../ticketing/scope-ticket.md).
- **Execution ticket** — holds the DAG and **owns the work as children**; active until the flow is defined, then advances as builds land. Format → [execution-ticket](../ticketing/execution-ticket.md).

```mermaid
graph TD
  MIL["milestone"] --> SCOPE["scope ticket<br/>what/why · done-by-def · never archived"]
...
**State conventions:** scope → **done** on its first child, never archived; execution → **active** while the flow forms; spike → **done** once it has a *result* ("a decision is still needed" is a valid result — the open decision then rides on the build the spike defined); build → **Pending → Ready → active → in review → done**, wired by explicit dependency edges; superseded work → archived **with a pointer**, never silently dropped.

**Restructure cleanly when the model shifts.** A concept earning its own milestone, or a decision re-partitioning the work, is the calibration loop firing — expected. Extract the new milestone, sweep *every* affected doc and ticket, re-file work, archive duplicates with supersession pointers.

**Human ratifies at the forks; the agent sweeps the mechanics.** Substantive tracker writes (new tickets in a real project, archivals, milestone boundaries) are proposed for approval; the mechanical sweep (renames, re-files, bulk creation) fans out to subagents. And **verify against live state**: re-derive by querying the tracker + the code, not forwarded prose; mark inferences `[confirm]`.
...
2. **Carry the *why* into the survivor.** The replacement's `## Evidence` records what it subsumed; the removed ticket goes **archived with a pointer** to its replacement.
...
At execution close (last build's change open), spend five minutes:
...
Record a one-line takeaway at the bottom of the execution ticket. You are systematically biased in some direction; explicit calibration shrinks the bias over executions.

**Calibration's other output: recipes.** Tuning priors is the inward half. The outward half: when the execution contained a recurring task whose path was non-obvious, **distill a recipe** — the wrong assumptions and fixes recorded across the execution become a "how we do X here" doc the next plan consults → [recipe](../recipe/SKILL.md). In a planned DAG this runs as the gate-side **distill node** feeding the completion gate — ticketed, with its input segment named — not an offline ritual ([execution-ticket](../ticketing/execution-ticket.md) → "Distill nodes").
